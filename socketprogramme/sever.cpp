    ...
    while (running) {
        ...
        if (iResult > 0) {
            ...
            if (receivedMessage == "quit") {
                ...
                // Lấy đường dẫn ứng dụng từ lệnh
                std::string appPath = receivedMessage.substr(7); // Cắt chuỗi để lấy đường dẫn
                std::wstring wAppPath = std::wstring(appPath.begin(), appPath.end());
